...
{
echo $service_cluster_instance_class
}
...
{
# Creating
echo "tanzu service class-claim create ${service_obj_name} --class ${service_cluster_instance_class:-${service_obj_name}};"

# Labeling
echo "kubectl label classclaim ${service_obj_name} backstage-dashboard-name=${service_shortname} backstage-dashboard-category=${service_category} backstage-dashboard-type=binding --overwrite;"
}
...
if [ ! -z ${service_link} ]
then
echo "kubectl create configmap ${service_obj_prefix}-${service_category}-${service_linkname} --from-literal=link=${service_link} --from-literal=link_description=\"${service_linkdescription}\" -oyaml --dry-run=client | kubectl apply -f -;"
echo "kubectl label configmap ${service_obj_prefix}-${service_category}-${service_linkname} backstage-dashboard-name=${service_linkname} backstage-dashboard-category=${service_category} backstage-dashboard-type=console --overwrite;"
fi
}
